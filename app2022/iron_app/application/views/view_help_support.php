                                ...
                                <div class="table-responsive">
                                    <table id="file_export" class="table table-striped table-bordered display">
                                        ...
                                        $i=1; foreach($view as $row){?>
                                            ...
                                            <td><?php echo $row->conteact;?></td>  
                                       
                                              <!--<td><?php echo $row->message;?></td> --> 
                                              <td>   <p  onclick = 'privew("<?=  base64_encode($row->message) ?>")' class = "btn"  > <?=  substr_replace($row->message,0,50) ?></p>
											      </td>  
                                              
                                              
                                              <td> 
											  <?php if(is_null($row->feedback)){ ?>
											    <button calss ='btn btn-info btn_reply' data = "<?= $row->h_id ?>" onclick = "view_fun(this)"  >Reply</button>
										 <?php }else{ ?>
...
{ xmlhttp = new XMLHttpRequest();
   ...
   <script src="<?php echo base_url();?>material/dist/js/pages/datatable/datatable-advanced.init.js"></script>
    ...
    $('body').on('submit','#mForm',function(e){
         ...
         $.ajax({
            type: 'post',
            url: "<?= base_url('Blog/contact_feedback_add_app');?>",
            data:$(this).serialize(), 
            async:false,
