                        ...
                        <div class="d-flex align-items-center">
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">View Referrals</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                    <div class="col-7 align-self-center">
          <a href="<?php echo base_url();?>User"><button class="btn btn-info" style="float:right;">View Users</button></a>
                    </div>
                </div>
            </div>
            <!-- ============================================================== -->
            ...
            <div class="container-fluid">
                <!-- ============================================================== -->
                 ...
                 <form method="GET" style="margin-bottom:10px;" action="<?php echo base_url()?>Referrals/search">
            <div class="col-md-12">
             <div class="row">
               <div class="col-md-3">
                        <label>Users</label>
           
             <input type="text" id="search-box" name="usr" class="form-control" autocomplete="off" placeholder="User Name">
...
foreach($selconten->result() as $rowte) {
 ...
 if(!empty($sdate)){  $datt = "AND referrals_date between "."'".$sdate."'"." AND "."'".$edate."'"; 
 }
...
$PageSqls = $this->db->query("SELECT * FROM dlc_referrals where referrals_id > 0 $datt $teid $userids");
...
$previouspage = $curpage - 1;
 $selcon = $this->db->query("select * from dlc_referrals where referrals_id > 0 $datt $teid $userids LIMIT $start, $perpage");
foreach($selcon->result() as $row) {?>
                                     ...
                                     <tr>
                                                ...
                                                <?php 
                                                 $tousrid = $row->to_user;
                                                 $usrid = $row->user_id; 
                                                 $alphid = $row->alpha_referrals_id;

                                             $sqll = $this->db->query("select * from dlc_user where user_id = '$usrid'"); 
                                              foreach($sqll->result_array() as $rw)
                                                  {
                                                ?>
                                                <td><?php echo $rw['user_fname'];?> <?php echo $rw['user_lname'];?></td>
                                                <td><?php echo $rw['user_email']; ?></td>
                                                <td><?php echo $rw['user_contact']; ?></td>
                                               
                                              <?php }?>
                                              
                                                <td><?php
                                               $sqll = $this->db->query("select * from dlc_referrals where user_id = '$usrid'");
                                               // echo $cnt = $sqll->num_rows();
                                                 ?>
                                          <a href="<?php echo base_url();?>Referrals/selec_referrals_as_per_user/<?php echo $usrid ?>"><?php echo $cnt = $sqll->num_rows();?></a>
                                                 </td>
                                                <td> <a href="<?php echo base_url();?>Free_courses/add_free_coursess/<?php echo $usrid; ?>"><button class="btn btn-success">Allot Free Courses</button></a></td>
...
$.ajax({
type: "POST",
url: "<?php echo base_url();?>Referrals/listing",
...
$.ajax({
type: "POST",
url: "<?php echo base_url();?>Referrals/friendsearch",
data:'keyword='+$(this).val(),
beforeSend: function(){
