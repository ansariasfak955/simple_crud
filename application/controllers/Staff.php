 ...
 public function index(){
  ...
  public function add_staff(){
    ...
    if(empty($fname) || empty($lname)  || empty($email)  || empty($mobile) ){
     ...
     $branch_id =  $this->session->userdata('branch_id');
     $data['list_doc']=$td = $this->cm->get_data_orderBy('doc_attachment_tbl',
                        ['branch_id'=>$branch_id,'user_id'=>$id,   'is_delete'=>'0'],'doc_id desc');
     ...
     public function add_timecard(){
       ...
       for($i=0 ; $i< count($class_id); $i++){
        ...
        // `staff_timecard_tbl`(`id`, `staff_id`, `in_time`, `out_time`, `days`, `time`, `date`class_id section_id   
           $arr[] = $this->cm->save('staff_timecard_tbl',['staff_id' =>$staff_id ,'in_time' =>$timein[$i],'out_time' =>$timeout[$i],
                        'class_id' =>$class_id[$i],'section_id' =>$section_id[$i],'time' =>$ad_time,'days' =>$days] );
